<?php 
    ...
    if(!class_exists('Youtube_Recommendation')){

        class Youtube_Recommendation{
            public $options;

            public function __construct(){
                $this->options = get_option('my_yt_rec'); // Carregando todos os "options" do plugin
            ...
            private function build_html_videos_list() {
                ...
                $custom_css     = strip_tags($custom_css); // Retira as tags HTML e PHP de uma string
                $custom_css     = htmlspecialchars($custom_css, ENT_HTML5 | ENT_NOQUOTES | ENT_SUBSTITUTE, 'utf-8'); // Converte caracteres especiais para a realidade HTML
    
                $language 	    = get_locale(); // Busca a localização configurada no Wordpress.
            ...
            public function enqueue_assets() {
                ...
                wp_localize_script( 'my-youtube-recommendation-scripts', 'my_yt_rec_ajax', array( 'url' => network_admin_url( 'admin-ajax.php' ))); // Escreve tag Javascript com valores vindos do PHP
